...
class NetworkDataSource(
    private val productApiService: EqualPricingApi = EqualPricingApiService.getInstance().retrofitService,
) : BaseNetworkDataSource() {

    suspend fun getProduct(productName: String): EqualResult<RemoteProduct> {
        return safeApiCall { productApiService.getProduct(productName) }
    }
